...
{#if submenu === 0 || submenu === -1}
  <Accordion name={language.longTermMemory} styled disabled={submenu !== -1}>
    ...
    {#if DBState.db.hanuraiEnable}
      <span class="mb-2 text-textcolor2 text-sm text-wrap wrap-break-word max-w-full"
        >{language.hanuraiDesc}</span
      >
    ...
    {:else if DBState.db.supaModelType !== 'none' && DBState.db.hypav2 === false && DBState.db.hypaV3 === false}
      ...
      <div class="flex mb-4">
        <Check
          bind:check={DBState.db.hypaMemory}
          name={language.enable + ' ' + language.HypaMemory}
        />
      </div>
    {/if}
