...
describe('BotSettings model-role provider visibility', () => {
  ...
  it('debounces provider draft keys before loading remote catalogs', () => {
    const source = botSettingsSource()

    expect(source).toContain('const PROVIDER_CATALOG_KEY_DEBOUNCE_MS = 400')
    expect(source).toContain('let nanogptCatalogApiKey = $state(nanogptKeyDraft.value)')
    expect(source).toContain('let openrouterCatalogApiKey = $state(openrouterKeyDraft.value)')
    expect(source).toContain('let ollamaCloudCatalogApiKey = $state(ollamaApiKeyDraft.value)')
    expect(source).toContain('nanogptCatalogApiKey = nextApiKey')
    expect(source).toContain('openrouterCatalogApiKey = nextApiKey')
    expect(source).toContain('ollamaCloudCatalogApiKey = nextApiKey')
    expect(source).toContain('getNanoGPTModelCatalog(')
    expect(source).toContain('nanogptUseSubscriptionEndpointDraft.value')
    expect(source).toContain("getOllamaModels(ollamaURLDraft.value, 'cloud', ollamaCloudCatalogApiKey)")
    expect(source).toMatch(/getOpenRouterModels\(\s*\{\s*apiKey:\s*openrouterCatalogApiKey,?\s*\},?\s*\)/)
    expect(source).toContain('<OpenrouterSettings apiKey={openrouterCatalogApiKey} />')
    expect(source).toContain('apiKey={nanogptCatalogApiKey}')
    expect(source).toContain('currentApiKey={nanogptKeyDraft.value}')
    expect(source).not.toContain('Promise.all([getNanoGPTModels')
  })
})
