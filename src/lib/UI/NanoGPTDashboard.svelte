          ...
          {:else}
            {#if subscription.cancelAtPeriodEnd}
              <p class="text-xs text-yellow-400">
                {language.nanoGPTCancelsAtPeriodEnd(fmtDate(subscription.period?.currentPeriodEnd))}
              </p>
            {/if}
            ...
            {#if subscription.period}
              <p class="text-xs text-textcolor2">
                {language.nanoGPTRenewsLabel}
                <span class="text-textcolor">{fmtDate(subscription.period.currentPeriodEnd)}</span>
              </p>
            {/if}
          {/if}
        {/if}
