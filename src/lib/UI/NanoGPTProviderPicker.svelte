    ...
    <div class="mt-2 flex flex-col gap-1.5">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
        ...
        {#each data.providers.filter((p) => p.available) as p}
          {@const inBadge = p.comparison
            ? discountBadge(
                p.comparison.platformVsOfficial.inputDiscountPct,
                p.comparison.platformVsOfficial.inputDirection,
              )
            : null}
          {@const outBadge = p.comparison
            ? discountBadge(
                p.comparison.platformVsOfficial.outputDiscountPct,
                p.comparison.platformVsOfficial.outputDirection,
              )
            : null}
          <button
            onclick={() => {
            ...
            <div class="mt-1.5 flex flex-col gap-0.5 text-xs text-textcolor2">
              <span
                >{language.nanoGPTProviderQuantization}:
