...
<div class="h-full w-full flex flex-col overflow-y-auto items-center">
  ...
  <div class="w-full flex p-4 flex-col text-textcolor max-w-4xl">
    {#if !$OpenRealmStore}
      ...
      {/if}
      <div class="mt-4 mb-4 w-full border-t border-t-selected"></div>
      <h1 class="text-2xl font-bold mb-4">Related Links</h1>
      <div class="grid w-full grid-cols-1 gap-4 p-2 md:grid-cols-2">
        {#each relatedLinks as relatedLink}
          <button
            class="group relative flex min-h-[140px] flex-col justify-center overflow-hidden rounded-2xl border border-borderc/10 bg-darkbg p-6 text-left transition-all duration-300 hover:-translate-y-1 hover:border-borderc/30 hover:bg-selected/50 hover:shadow-xl hover:shadow-darkbg/50"
            onclick={() => {
