...
<div class="list flex flex-col rounded-md border border-selected">
  {#each list as n, i}
    <div class="w-full h-10 flex items-center">
      ...
      <button
        aria-label={`${language.moveUp}: ${language.formating[n]}`}
        class="mr-1"
        disabled={list.length < 2}
        onclick={() => moveItem(i, -1)}><ChevronUp /></button>
      <button
        aria-label={`${language.moveDown}: ${language.formating[n]}`}
        class="mr-1"
        disabled={list.length < 2}
        onclick={() => moveItem(i, 1)}><ChevronDown /></button>
    </div>
    {#if i !== list.length - 1}
