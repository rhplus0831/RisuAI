...
{#if isExpanded}
  <div class="absolute w-full h-full z-40 flex justify-center items-center pointer-events-none">
    <button
      ...
      <span class="mb-6 text-left text-gray-500 text-sm">{$hypaV3ProgressStore.msg || ''}</span>
      <div class="w-full min-w-64 md:min-w-138 h-2 bg-darkbg border border-darkborderc rounded-md">
        <div class="h-full bg-linear-to-r from-blue-500 to-purple-800 saving-animation transition-[width]"></div>
      </div>
      <span class="w-full mt-6 text-center text-gray-500 text-sm">{$hypaV3ProgressStore.subMsg || ''}</span>
    </button>
  </div>
{:else}
  <button
