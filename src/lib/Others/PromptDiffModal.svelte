<script lang="ts">
  ...
  type SplitRow =
    | {
        kind: 'divider'
        seg: Extract<DiffSegment, { kind: 'divider' }>
        scope: string
        key: string
      }
  ...
  const nameHeaderTagClass =
    'shrink-0 text-[10px] px-1.5 py-0.5 rounded-sm border border-white/15 text-textcolor2 bg-black/20'

  const splitEmptyLineClass =
    `${diffLineBase} ${diffLineCommon} ` +
  ...
  async function computeCardViewDiff(
    prompt1: PromptCard[],
    prompt2: PromptCard[],
    style: DiffStyle,
  ): Promise<CardDiffResult> {
    ...
    for (const part of summary.parts) {
      ...
      if (part.k === 'add') {
        ...
        const bodyLines = await computeDiffLineByLine([], cardLines, style)
  ...
  async function computeDiffCardLine(
    prompt1: PromptCard[],
    prompt2: PromptCard[],
    style: DiffStyle,
  ...
  async function computeDiffLineByLine(
    prompt1: PromptLine[],
    prompt2: PromptLine[],
    style: DiffStyle,
  ): Promise<DiffResult> {
...
{#snippet renderSimpleLine(part: NonModifyPart)}
  <div class={lineClassOf(part)} class:mt-5={isNameLine(part)} class:mb-5={isHeaderLine(part)}>
    ...
    <div class={`whitespace-pre-wrap ${lineRemoveClass}`}>
      {#each parts as part, i (i)}
        {@render renderTokens(part.tokens, 'add', tokenPackLineRemove)}{#if tagText(part)}<span
            class={nameHeaderTagClass}>{tagText(part)}</span
          >{/if}
    ...
    <div class={`whitespace-pre-wrap ${lineAddClass}`}>
      {#each parts as part, i (i)}
        {#if part.src === 'linebyline' && part.right.text === ''}
          <span class="text-textcolor2/60 italic">[empty line]</span>
        {:else}
          {@render renderTokens(part.tokens, 'remove', tokenPackLineAdd)}{#if tagText(part)}<span
              class={nameHeaderTagClass}>{tagText(part)}</span
            >{/if}
        {/if}
...
{#snippet renderSplitCell(cell: SplitCell, idx: number)}
  ...
  {:else}
    ...
    {:else}
      {@const tag = tagText(part)}
      {@const sideText = isLineby(part) ? (isLeft ? part.left.text : part.right.text) : null}
    ...
    <div class="flex items-center justify-between px-4 py-3 border-b border-darkborderc">
      <div class="flex items-center gap-4 flex-wrap">
        ...
        {@render pillRadioGroup(
          ...
          formatOptions,
          formatStyle,
          (v) => (formatStyle = v as FormatStyle),
          isFlatText,
        )}
        {@render pillRadioGroup(
          'View',
