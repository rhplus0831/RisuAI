<script lang="ts">
    ...
    en: [
      {
        ...
        tip: 'Iris can access various data through the Risuai system. It uses ax model defined in config.',
      },
    ],
    ko: [
      {
        speaker: 'Iris',
        text: '안녕하세요. 아이리스라고 합니다~.',
        tip: '아이리스는 보조 모델을 사용하며, Risuai의 전반적인 데이터에 접근할 수 있습니다.',
      },
    ],
    'zh-Hant': [
      {
        speaker: 'Iris',
        text: '你好，我一直在等你。',
        tip: 'Iris 可以通過 Risuai 系統訪問各種數據。它使用配置中定義的輔助模型。',
      },
    ],
  }
  ...
  const unsupportedModelDialogue: Record<string, DialogueLine[]> = {
    en: [
      {
        speaker: 'Iris',
        text: "It seems your current model doesn't support me responding... Please switch to a compatible model, like GPT, Claude, or Gemini which are not plugins.",
      },
    ],
    ko: [
      {
        speaker: 'Iris',
        text: '현재 모델이 제가 응답하는걸 지원하지 않는 것 같아요. 플러그인이 아닌 GPT, Claude, Gemini 모델로 전환해주세요.',
      },
    ],
    'zh-Hant': [
      {
        speaker: 'Iris',
        text: '看起来您当前的模型不支持我响应... 请切换到兼容的模型，如 GPT、Claude 或 Gemini，这些都不是插件。',
      },
    ],
  }
...
<div
  ...
  <!-- Dialogue box -->
  <div class="relative mb-6 w-full max-w-4xl px-4" transition:fly={{ y: 40, duration: 300 }}>
    <div class="relative rounded-xl border border-white/20 bg-black/80 px-8 py-6 shadow-2xl">
      <!-- Backlog button -->
      <button
        onclick={openBacklog}
