...
const readFileInsecure = isTauri
  ? readFile
...
const writeFileUnSecure = isTauri
  ? writeFile
  : (path: string, data: Uint8Array, options: any) => {
...
async function fetchProtectedResourceSPA(
  url: string,
  options: RequestInit = {},
  arg: ProtectedResourceArg = {},
) {
  ...
  if (!tokenInitalized || arg.forceRefresh) {
    const oauthDataText = await readFileInsecure('oauthData.json', {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        const extractable = await crypto.subtle
          .exportKey('jwk', dPoPKeyPair.privateKey)
          .then(() => true)
          .catch(() => false)
        if (extractable) {
...
async function fetchProtectedResourceRisuAuthVersion(
  url: string,
  options: RequestInit = {},
  risuAuth: string,
  arg: ProtectedResourceArg = {},
