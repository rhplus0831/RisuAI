...
describe('browser prompt-assembly model ownership', () => {
  ...
  it('resolves the main profile once and passes its model into prompt-shape consumers', () => {
    const assembly = source('src/ts/process/sendChatPromptAssembly.ts')
    expect(assembly).toContain("resolveModelProfile({ database: getDatabase(), role: 'chatMain' }).modelId")
