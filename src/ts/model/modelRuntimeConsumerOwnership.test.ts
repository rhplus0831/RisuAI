...
describe('browser model-runtime consumer ownership', () => {
  it('keeps tokenizer budgeting off the flat main-model field', () => {
    ...
    expect(tokenizer).toContain('encode(data.content, this.profile, this.tokenizerSelection, this.database)')
  ...
  it('routes prompt-visible model identity through resolved role contexts', () => {
    ...
    expect(browserParser).toContain('resolveModelProfile({ database: parserRuntimeDatabase(), role })')
    ...
    expect(pluginApi).toContain('resolveModelProfileWithLegacyCompatibility({')
    expect(pluginApi).toContain('database: currentPluginDatabaseSnapshot() as unknown as Database')
