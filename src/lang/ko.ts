...
export const languageKorean = {
  ...
  help: {
    ...
    agentPresetPreparedInputs:
      '준비된 입력은 표시된 CBS 자리표시자(예: `{{currentUserMessage}}`)가 단계 지침에 삽입된 경우에만 해당 에이전트 단계용 컨텍스트로 수집됩니다. 단계 지침에서 해당 내용이 들어갈 위치에 자리표시자를 넣으세요. 최대 입력 문자 수는 수집되는 내용에도 적용됩니다. 메인 초안은 메인 응답 이후 단계에서만 사용할 수 있습니다.',
  ...
  agentPresets: {
    ...
    settingsDescription: '채팅 생성에 재사용할 수 있는 보조 에이전트 실행 계획입니다.',
    ...
    beforeMain: '메인 응답 전',
    afterMain: '메인 응답 후',
    stepCount: (count: number) => `활성 단계 ${count}개`,
    ...
    preparedInputScopesDescription:
      '이 단계에서 사용할 수 있는 컨텍스트를 선택하세요. 선택한 입력은 해당 지침 자리표시자가 있는 위치에만 포함됩니다.',
    ...
    inputScopeDescriptions: {
      ...
      previousAgentOutputs:
        '실행 구간, 출력 키 및 단계 이름과 함께 표시되는, 사용 가능한 이전 에이전트 단계의 완료된 출력입니다.',
      mainDraft: '출력 편집 후의 메인 모델 초안입니다. 메인 응답 이후 단계에서만 사용할 수 있습니다.',
    },
    invalidOutputKey: '영문자, 숫자 또는 밑줄을 사용하고 영문자나 밑줄로 시작하세요.',
    savePresetBeforeSteps: '단계를 편집하기 전에 프리셋을 저장하세요.',
    outputKey: '출력 키',
    diagnostics: '진단',
    ...
    deleteStepConfirm: (name: string) => `"${name}" 단계를 삭제하시겠습니까?`,
