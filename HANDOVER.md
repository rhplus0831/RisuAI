...
Date: 2026-05-24
Branch: `fastify`
Head: `2a122674 docs: lock Phase 8/9 pre-design decisions and re-slice remaining sizing gaps`
...
| 7-12a   | `a3a1e6e2` | Browser client adapter (`serverChat.ts` `requestServerChat`): POSTs `AssembleInput` to `/chat` with `risu-auth`, stream-parses `stage`/`prompt`/`info`/`error`/`done` (ignores dispatch-coupled events), returns `{ status: 'ok'\|'error'\|'aborted' }`. Shared `sseParse.ts`; introduced the `db.useServerPromptAssembly` gate used by 7-12c. |
...
- `server/fastify/src/routes/generationChat.ts` registers two routes.
  ...
  `error` + `done` (no `info`). `POST /api/v1/generate/preview-prompt`
  (7-11h) is the one-shot **JSON** variant: it returns the
  `result.prompt` payload, `{ stopSending, abortReason }`, or a real
  HTTP **404** for `EntityNotFoundError` (no SSE head). Both bind
  `loadDatabase` to `loadPersisted(dataDir).database` (read-only —
  `varChanged` persistence + provider dispatch are 7-12).
...
- `assemble.ts` holds the 7-11a–f assembler steps. `beginAssembly`
  ...
  is now **closed** by 7-11i (the `info` SSE telemetry on `/chat`).
- `src/ts/process/request/serverChat.ts` (7-12a, Tier 4) is the browser
  client adapter for `/chat`: `requestServerChat(input, signal)` POSTs the
  `AssembleInput` body with the `risu-auth` header, stream-parses the named
  SSE events via the shared `sseParse.ts` (`parseSseEvent` /
  `iterateSseEvents`, lifted out of `serverCompletion.ts`), and returns
  `{ status: 'ok', prompt, info } | { status: 'error', error } | { status: 'aborted' }`.
  It consumes `stage` / `prompt` / `info` / `error` / `done`
  ...
  independent of `useServerGeneration`) is wired for the preview paths in
  7-12c; send / continue / regenerate wait for 7-12d.
  `serverChatEvents.ts` mirrors the locked `PromptChatEvent` union
  client-side.
...
- Ooba OAI-compatible, NovelAI text, and NovelList remain local-only
  until the Phase 7 closeout / server-owned flattening follow-up.
