...
export function normalizeAllCharacterChats(database: unknown): CharacterRecord[] {
  ...
  // Anti-pattern: folder ids are normalized per-character, so two characters can
  // collide on the same folder id. Repair must run globally across characters.
  for (const character of characters) {
    normalizeGlobalChatFolderIds([character])
  }
  return characters
}
