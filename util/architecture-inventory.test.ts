...
describe('cross-runtime baseline gate', () => {
  it('matches the reviewed repository baseline and records every current lane', () => {
    ...
    expect(observation.edges.reduce((total, edge) => total + edge.count, 0)).toBe(74)
    expect(
      Object.fromEntries(
    ...
    ).toEqual({ production: 59, 'server-test': 15, 'browser-smoke': 0 })
  }, 15_000)
...
describe('client resource ownership gate', () => {
  ...
  it('matches every reviewed consumer, bridge family, and temporary seam', () => {
    ...
    expect(observation.consumers.reduce((total, consumer) => total + consumer.count, 0)).toBe(9887)
