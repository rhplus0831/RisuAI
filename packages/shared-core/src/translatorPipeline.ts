...
import { replaceHistorySlots, type HistorySlotResolver } from './historySlots.js'
...
function parseTranslatorChatML(data: string): TranslatorChatMessage[] | null {
  ...
  return trimmed
    ...
    .filter(Boolean)
    .map((part) => {
      let role: TranslatorChatMessage['role'] = 'user'
      let content = part
