import { LLMFormat } from './modelTypes.js'
import { isMaskedProviderSecret } from './providerSecretMask.js'

/**
 * Single source of truth for provider-routing capability. Given a resolved
 * model and the narrow configuration fields used by its gates, it returns the
 * provider adapter or a stable unsupported category.
 *
 * This module is pure: it reads only its input and performs no environment,
 * aggregate-state, or I/O access. Callers own model-info derivation and map
 * stable reason categories to user-facing prose.
 */
...
/**
 * Only the config fields the capability gates read. Callers derive this narrow
 * view from their authoritative settings so the table documents exactly what
 ...
 * exists for the format. This is the decision table's first column.
 */
 ...
 * locally dispatched formats carry their own code so consumers can reproduce
 * their specific per-format message; everything else is the generic
 * `format-not-server-routable`.
 */
...
/**
 * Whether a reverse_proxy config carries the proxy URL + key its adapters need.
 * Shared by the OAI-compat / Anthropic / Mistral / Cohere / Responses /
...
function hasBedrockCredentials(claudeAPIKey: string | undefined): boolean {
  ...
  // Public settings projections intentionally expose only this exact marker.
  // Treat it as proof that a credential exists; the trusted execution owner
  // reloads and validates the real value before dispatch.
 ...
 * config refinement). All runtime consumers use this same classifier before
 * provider-specific wire payload construction.
 */
