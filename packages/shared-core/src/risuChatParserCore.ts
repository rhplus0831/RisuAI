import { calculateString } from './calculation.js'
import { registerCBS } from './cbsRegistry.js'
import { legacyBlockMatcher, parseArray, risuEscape, trimLines, type blockMatch } from './risuChatParserHelpers.js'
import type {
  CbsCallbackMemo,
  CbsCharacter,
  CbsDatabase,
...
const emptyChatVariables: RisuChatParserVariableResolver = {
  getChatVar: () => '',
  getGlobalChatVar: () => '',
}
...
export function matcher(
  ...
  chatVariables: RisuChatParserVariableResolver = emptyChatVariables,
):
  | {
      text: string
  ...
  try {
    if (p1.startsWith('? ')) {
      ...
      return calculateString(substring, chatVariables)?.toString() ?? null
    }
...
export function risuChatParser(da: string, arg: RisuChatParserArg = {}): string {
  ...
  const db = arg.db ?? arg.parserState?.getDefaultDatabase() ?? null
  const aChara = arg.chara
  let chara: CbsCharacter | string | null = null
  ...
  if (arg.tokenizeAccurate) {
    const db = arg.db ?? arg.parserState?.getDefaultDatabase() ?? null
    const selchar = chara ?? db?.characters[arg.parserState?.getDefaultSelectedCharID() ?? 0]
    if (!selchar) {
      chara = 'bot'
    }
  }
  ...
  const chatVariables = arg.chatVariables ?? emptyChatVariables
  let functions: Map<
    string,
    {
      data: string
  ...
  while (pointer < da.length) {
    switch (da[pointer]) {
      ...
      case '}': {
        ...
        const mc = isPureMode() ? null : matcher(dat, matcherObj, tempVar, chatVariables)
        if (!mc && mc !== '') {
          nested[0] += `{{${dat}}}`
