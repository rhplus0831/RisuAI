...
| --- | --- | --- | --- | --- |
| Create one durable, idempotent accepted-send operation | [MS-01](validate/ms-01-accepted-send-recovery.md), [MS-02](validate/ms-02-chat-level-activity-is-not-operation-ownership.md) | Ready | A user row can be durably accepted while the obligation to launch or recover its reply exists only in browser memory and disappears on reload or process eviction. | Add an operation/message identity before append, atomically persist append plus generation intent, and launch provider work after commit. This is the foundation for MS-02, MS-04, MS-07, and MS-10. Direction decided 2026-08-11: server-side atomic operation (single idempotent endpoint owning append+intent in one transaction); a client-only journal is rejected as the primary mechanism. Design complete: [protocol design](plans/accepted-send-protocol.md) waves 1-3. Wave 1 (ledger, boot sweep, bootstrap projection) landed in `43247b49e`; wave 2 (atomic submit/status/stream/cancel/retry endpoints, job binding, finalization transitions) landed in `09b70cc6f`. Browser cutover (wave 3) pending. |
| Carry exact send lineage through jobs and transfer same-chat ownership through handoff | [MS-02](validate/ms-02-chat-level-activity-is-not-operation-ownership.md), [MS-01 Addendum A](validate/ms-01-accepted-send-recovery.md), [MS-02 A-01](validate/ms-02-chat-level-activity-is-not-operation-ownership.md), [MS-07 retry addendum](validate/ms-07-recovered-job-warning-coexistence.md) | Ready | An unrelated same-chat job or a newer user row can be credited to an older send; retry can clear the wrong recovery, and an early lease release permits duplicate sends. | Carry operation/message IDs through POST, job, SSE, bootstrap, reattach, finalization, and recovery. Transfer the preparation reservation until the exact operation owns a job, recovery, or terminal result. Design complete: [protocol design](plans/accepted-send-protocol.md) waves 2-3. Wave 2 landed in `09b70cc6f`: SSE frames, job_accepted, bootstrap, finalization journals, and command events all carry operation lineage; same-chat claims are unified across protocol and legacy operations. Browser-side consumption (wave 3) pending. |
| Make Stop order-independent and acknowledged before and after job-ID delivery | [MS-04](validate/ms-04-pre-job-id-cancellation-gap.md), [MS-04 post-ID addendum](validate/ms-04-pre-job-id-cancellation-gap.md), [MS-08 AV-01](validate/ms-08-reattach-exhaustion.md) | Ready | Stop can appear successful while the provider continues and a full or partial unwanted reply is later persisted. | Reuse the send operation identity; retain cancel-before-POST tombstones, persist unacknowledged Stop intent, return typed cancellation outcomes, and reconcile until the exact operation is absent or completed. UX decided 2026-08-11: explicit acknowledged "Stopping…" state with retryable failure, not optimistic instant stop. Design complete: [protocol design](plans/accepted-send-protocol.md) wave 4. |
| Prevent stale reattach restoration from outranking a newer same-chat job | [MS-02 A-03](validate/ms-02-chat-level-activity-is-not-operation-ownership.md) | Ready | A failed stale reattach can be restored beside a newer job, causing Stop to target the stale job while the new job continues and persists. | First pin the timing sequence. Then make restoration projection-epoch-aware or at least same-chat-deduplicated; exact job cancellation must select the authoritative newer job. Design complete: [protocol design](plans/accepted-send-protocol.md) wave 5. |
| Define the server-restart outcome for in-flight generation | [Explicit design limitation](fastify-multichat-mobile-stability-audit-2026-08-11.md#server-restart-loses-in-flight-generation-ownership) | Ready | A restart loses the runner, replay, lock, and bootstrap projection after a send may already be durable. | Decision 2026-08-11: authoritative abandoned/retryable operation state (no automatic resume/regeneration; avoids double provider billing and surprise generation). On restart, ledger operations with no terminal result are marked abandoned and projected through bootstrap as a retryable state. Design depends on the accepted-send ledger. Design complete: [protocol design](plans/accepted-send-protocol.md) waves 1-2 (boot sweep + bootstrap projection). Wave 1 landed in `43247b49e`: boot sweep marks interrupted operations abandoned (never auto-resumed) with providerMayHaveRun, and bootstrap projects them; the retryable UI arrives with wave 3. |
...
| Make remaining unsupported trigger/CBS paths explicit and non-throwing | [Contextual trigger/CBS finding](fastify-multichat-mobile-stability-audit-2026-08-11.md#contextual-migration-spot-checks) | Resolved | Non-persistent effects could silently no-op and browser-context CBS callbacks could throw under Fastify. | Decision 2026-08-11 implemented in `67210c623`: browser-language and screen-width CBS resolve server-side from the per-request reported client context (normalized/bounded); missing context and the deliberately unsupported `screenheight` return empty values with structured non-throwing diagnostics converted to localized warnings. Regression: `src/ts/process/request/clientContext.test.ts`, `src/ts/process/request/tests/serverChat.test.ts`, `server/fastify/__tests__/promptVariables.test.ts`. |
