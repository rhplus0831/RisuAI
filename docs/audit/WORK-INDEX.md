...
causes, this index contains **35 active work items**, **1 already-resolved
contextual item**, and **0 deferred items**.
...
| Reconstruct, render, and eventually settle real finalization-queue state | [MS-05 A-1/A-2/A-6](validate/ms-05-generation-finalization-journal.md) | Needs design | A legitimately provisional reply has no row indicator, so users can mistake it for durable data; its marker is lost on reload, it may disappear and reappear, and it can retry forever without a visible stalled policy. | Project writer-scoped pending/terminal retries through bootstrap or a resource, render the existing localized state, and decide bounded backoff plus terminal/stalled behavior. |
...
| Define the server-restart outcome for in-flight generation | [Explicit design limitation](fastify-multichat-mobile-stability-audit-2026-08-11.md#server-restart-loses-in-flight-generation-ownership) | Needs decision | A restart loses the runner, replay, lock, and bootstrap projection after a send may already be durable. | Decide between a durable resumable queue/worker and an authoritative abandoned/retryable operation state. Build on the accepted-send ledger rather than another browser-only heuristic. |
...
| Require an exact assistant result before writing translated PO output | [MS-10 additional issue 2](validate/ms-10-auxiliary-send-recovery-bypass.md) | Ready | A failed generation can export the source user row as if it were the translated assistant result. | Require adjacency to the exact accepted message in the captured chat; use guarded authoritative reconciliation when the projection is missing and suppress success/download on failure. |
| Recover terminal-only durable and observable effects idempotently | [MS-06 AV-03](validate/ms-06-completion-reconciliation.md) | Needs design | A completion recovered only from the transcript can skip IGP and plugin automation, making interrupted and uninterrupted jobs produce different durable/observable outcomes. | Define an exact job/message effect ledger and per-effect idempotency. Decide explicit policies for ephemeral notification, TTS, and emotion work; prefer server ownership for durable effects. |
| Make optimistic character creation and import completion truthful | [Contextual optimistic-creation finding](fastify-multichat-mobile-stability-audit-2026-08-11.md#contextual-migration-spot-checks) | Needs design | Non-selecting create/import returns an ID or index before durable settlement, so an apparently created character can later disappear after rollback. | Await and return the durable outcome, or expose a queued/recovery state that owns rollback. Test navigation, rejection, and caller-visible result semantics. |
| Prevent unsupported persistent trigger effects from masquerading as durable mutations | [Contextual trigger/CBS finding](fastify-multichat-mobile-stability-audit-2026-08-11.md#contextual-migration-spot-checks) | Needs decision | Visible character, persona, and lorebook effects can silently no-op, so users can believe persistent data changed when it did not. | Preserve prior accepted retirements, but choose implementation versus configuration/import-time rejection for every persistent effect. Keep imported definitions intact, diagnose them before execution, and pin the selected contract. |
| Surface missing and orphaned assets in bundle-import results | [Contextual bundle-import finding](fastify-multichat-mobile-stability-audit-2026-08-11.md#contextual-migration-spot-checks) | Ready | The browser announces success while dropping the server's report that imported persistent data has missing/orphaned assets. | Parse `assetReport`, present a localized qualified result, and cover missing-reference and orphan cases without weakening the atomic import safeguards. |
| Flush the final PO entry at end-of-file | [MS-10 additional issue 4](validate/ms-10-auxiliary-send-recovery-bypass.md) | Ready | A valid final PO entry without a trailing blank separator is omitted, producing an incomplete exported artifact. | Flush the accumulated entry at EOF and add fixtures with and without trailing separators. |
...
| Work item | Findings | Status | Impact | Risk / dependencies |
| --- | --- | --- | --- | --- |
| Normalize the default/legacy Kobold URL | [Contextual Kobold finding](fastify-multichat-mobile-stability-audit-2026-08-11.md#contextual-migration-spot-checks) | Ready | The repository default ending in `/api/v1` becomes `/api/v1/api/v1/generate`, so the default legacy provider can target an invalid endpoint. | Make suffix joining segment-aware and test bare host, intermediate `/api/v1`, and full generate URL. |
| Implement or explicitly disable Ooba Legacy streaming | [Contextual Ooba finding](fastify-multichat-mobile-stability-audit-2026-08-11.md#contextual-migration-spot-checks) | Needs decision | `useStreaming` is calculated but the adapter always uses buffered HTTP, so a visible setting has no effect. | Decide whether WebSocket streaming remains supported; either implement and test it or disable the control with a localized compatibility notice. |
| Support or explicitly diagnose standalone `CHAT` save blocks | [Contextual save-import finding](fastify-multichat-mobile-stability-audit-2026-08-11.md#contextual-migration-spot-checks) | Needs decision | Saves using that compatibility block cannot be imported, although rejection is currently atomic rather than destructive. | Choose supported conversion versus a documented/import-time incompatibility; preserve validate-before-replace behavior. |
| Make remaining unsupported trigger/CBS paths explicit and non-throwing | [Contextual trigger/CBS finding](fastify-multichat-mobile-stability-audit-2026-08-11.md#contextual-migration-spot-checks) | Needs decision | Non-persistent effects can silently no-op and browser-context CBS callbacks can throw under Fastify. | After the Tier 0 persistent-effect decision, choose implement-versus-block for the remaining surfaces; add configuration/import diagnostics and safe server guards so unsupported callbacks do not crash generation. |
| Fix the PO extracted-note marker typo | [MS-10 additional issue 5](validate/ms-10-auxiliary-send-recovery-bypass.md) | Ready | `#. Note =` is recognized but `#. Notes =` is removed, leaving the marker in the model input and degrading translation correctness. | Normalize the accepted marker(s) and add singular/plural parser fixtures. |
...
| Decide consumer-only durable-stream cancellation semantics | [MS-04 hardening observations](validate/ms-04-pre-job-id-cancellation-gap.md) | Needs decision | Cancelling the returned token stream without the owner AbortSignal can detach local processing while the durable job continues. | Specify passive detach versus explicit Stop for this nonstandard path, then pin listener cleanup, progress, and server-job behavior. |
