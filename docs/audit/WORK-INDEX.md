...
| Implement or explicitly disable Ooba Legacy streaming | [Contextual Ooba finding](fastify-multichat-mobile-stability-audit-2026-08-11.md#contextual-migration-spot-checks) | Ready | `useStreaming` is calculated but the adapter always uses buffered HTTP, so a visible setting has no effect. | Decision 2026-08-11: disable the streaming control for Ooba Legacy and show a localized buffered-only compatibility notice; do not implement legacy WebSocket streaming unless demand is demonstrated. |
| Support or explicitly diagnose standalone `CHAT` save blocks | [Contextual save-import finding](fastify-multichat-mobile-stability-audit-2026-08-11.md#contextual-migration-spot-checks) | Ready | Saves using that compatibility block cannot be imported, although rejection is currently atomic rather than destructive. | Decision 2026-08-11: documented import-time incompatibility. Replace the raw error with a friendly localized diagnostic, document the limitation, and keep atomic validate-before-replace rejection; revisit conversion support only if affected saves are actually encountered. |
