...
implementation plan and has completed the durable contract/schema slice plus
the pure resolver/planner/status slice.

## Snapshot

- Plan state: Phase 1 implemented.
...
- Current implementation state:
  ...
  - `server/fastify/src/prompt/contextAgent.ts` still detects and fills
    `{{agent}}` / `{{slot::agent}}`.
- Current verification state: focused Phase 1 resolver/planner tests and
  strict client/server TypeScript checks passing.
...
- Implemented Phase 1 pure Agent Preset resolver/planner/status helpers and
