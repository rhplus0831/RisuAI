...
- test harnesses and currently unmounted surfaces: `LiteMain.svelte`,
  `WelcomeRisu.svelte`, `PromptDiffModal.svelte`, `PlaygroundRegex.svelte`, and
  the standalone `MobileBody`/`MobileHeader`/
  `MobileFooter` shell;
...
- `CHAT-10` — Per-message actions (`R`, `L`, `A`, `B`)
  - Variants: Desktop inline versus narrow-screen popup actions; copy, TTS, edit, delete, translate, bookmark, branch, disable-one/disable-above, and partial edit appear or disable by row role/index, settings, TTS/translator capability, generation/translation state, and disabled range. Partial edit triggers by hover, drag selection, or touch long-press, and routes to the original or persisted-translation layer per rendered block in translated/bilingual display.
  - Owners: `src/lib/ChatScreens/Chat.svelte`; `PartialEditController.svelte`; `partialEditLayer.ts`; `partialEditTouchTrigger.ts`
...
- `CHAT-12` — Composer and generation ownership (`R`, `A`, `L`)
  - Variants: Send versus abort/progress; character menu versus playground action; normal plus translated textarea; translation rollback; selected draft-hook output; BTW hook picker/pending/result/dismissed state; continue/regenerate disabled state. Message, translation, files, draft output, and BTW output are keyed by transcript identity. The persistent composer dock is a sibling of the reverse transcript scroller, so paging and user scrolling cannot move the editor. While a text editor is focused, the app shell follows the visual viewport and the document scroll guard yields vertical ownership. Only a generation owned by the visible chat gets visible progress, although another active generation can still disable sending.
...
- `CHAT-14` — Generation and post-generation progress (`A`, `R`)
  ...
  - Owners: `src/lib/ChatScreens/Chat.svelte`; `src/lib/ChatScreens/Chats.svelte`; `src/lib/ChatScreens/AgentPresetProgress.svelte`; `src/lib/ChatScreens/PostGenerationScriptProgress.svelte`; `src/lib/ChatScreens/chatGenerationLoading.ts`; `src/ts/process/halfStreamingProgress.ts`; `src/ts/process/postGeneration/streamResponse.ts`; `src/ts/process/agentPresetProgress.ts`; `src/ts/process/postGenerationProgress.ts`
...
Primary owners: `src/ts/process/modules.ts`, `src/ts/stores.svelte.ts`,
`src/lib/ChatScreens/BackgroundDom.svelte`, `src/lib/SideBars/Toggles.svelte`,
and `src/ts/parser/`.
...
Primary owners: `src/lib/ChatScreens/Chat.svelte`,
`src/lib/ChatScreens/ChatBody.svelte`,
`src/lib/ChatScreens/BackgroundDom.svelte`,
`src/lib/ChatScreens/ChatBodyParseMemo.ts`, `src/ts/parser/`, `src/ts/cbs.ts`,
