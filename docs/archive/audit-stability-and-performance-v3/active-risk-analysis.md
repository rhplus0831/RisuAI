...
- Gated items: unchanged from v2 — `v2-L12`, plus the v1 carry-overs (v1-L4,
  v1-L7, v1-L26, v1-U2) and the `../leftover.md` evidence gates. The
...
| L9  | [7](phases/phase-7-assembly-and-trigger-hot-paths.md)       | Bound user-regex execution (haystack/pattern caps or complexity screen); document non-interruptibility at minimum. | DONE |
| L10 | [7](phases/phase-7-assembly-and-trigger-hot-paths.md)       | Bump the history-callback memo generation from every chat-var-dirty fold (all three un-bumped sites). | DONE |
...
| L38 | [8](phases/phase-8-client-interpreters-plugins-media.md)    | Port the server `TriggerExecutionBudget` caps + abort to the client `runTrigger` (manual entrypoints). | DONE |
...
- v1 carry-overs (owned by the
  [archived v1 risk analysis](../audit-stability-and-performance/active-risk-analysis.md)):
...
- `../leftover.md` evidence gates: prompt-assembly hydrated load +
