...
Phase 3 later removed the redundant transcript clones.

Landed: `applyTailDataSwap` and `applyTailSlice` in
...
Phase 3 then replaced the old regenerate transcript reinstall path with
`applyRerollTruncate`, removing the redundant dispatch and full-transcript clones.
Proof: `rerollNavigation.rollback.test.ts` (clone-cost: a swipe never serializes
the large sibling transcript; rollback: a failed `dispatchReplaceMessagesScoped`
restores only the active chat). The unit and guard suites now assert the scoped
dispatch variants.
...
- `src/ts/process/rerollNavigation.svelte.ts` - `applyTailSlice`
  ...
  the dispatch re-cloned before Phase 3).
- `src/ts/process/rerollNavigation.svelte.ts` - `applyRerollTruncate` (Phase 3
  regenerate truncation path).
- `src/lib/ChatScreens/DefaultChatScreen.svelte` - `reroll()`/`unReroll()`
  swipe/gesture/side-menu bindings.

## Implemented Shape

- `applyTailDataSwap` and `applyTailSlice` use `currentChatScopedSnapshot()`.
...
- The redundant `safeStructuredClone(record.message)` and reroll full-transcript
  clone were intentionally left for Phase 3; current HEAD uses
  `applyRerollTruncate` for regenerate.

## Behavior / Invariants

- Swipe navigation (cycling buffered candidates through `applyTailSlice`) stays
  synchronous and correct; the buffered-candidate tail clones are bounded and
  untouched.
