...
| Resource state and reads        | `src/ts/server/characterShellHydration.test.ts`; `chatMessageHydration.reactivity.svelte.test.ts`; `chatMessageHydration.test.ts`; `hydrationReads.test.ts`; `promptTemplateHydration.test.ts`; `resourceCache.test.ts`; `resourceReads.test.ts`; `resourceState.svelte.test.ts`; `shellProtocol.test.ts`; `shellHydration.test.ts`; `resourceManifest.test.ts`; `routeResourceLoader.test.ts` |
...
| Fast-bootstrap browser evidence | `server/fastify/browser-smoke/phase0StartupMatrix.spec.ts`; `phase7IntegrationMatrix.spec.ts`; `fastBootstrapHarness.ts` |
