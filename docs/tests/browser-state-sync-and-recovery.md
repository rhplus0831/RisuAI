...
This is one of the suite's strongest and most important areas. Its 26 primary files contain 829
runtime-expanded cases. The audit run passed all 829 without skips or retries. Tests repeatedly enforce
the core safety rule: an accepted revision is either acknowledged against the exact, still-current
...
- `resourceInvalidation.test.ts` expands seven malformed Agent Preset events; settings group/full orders;
  ...
  and four malformed cases; plus seven broad fallbacks (revision gap, missing/unknown settings group,
...
- Agent/persona/translator: Agent Preset create/update/delete/duplicate/reorder/default and step create/
  update/delete/duplicate/reorder; persona create/delete/select/reorder; translator preset create/update/
  delete/select.
...
6. Split the 8,974-line command test by transport, queue/retry, local acknowledgements, and domain adapter
...
and UI files are discussed in their focused documents rather than counted here.

| Location                                                       |   Cases | Protected behavior                         |
| -------------------------------------------------------------- | ------: | ------------------------------------------ |
| `src/ts/bootstrap.test.ts`                                     |     140 | Full browser startup and reconciliation    |
| `src/ts/server/activeWriterSession.test.ts`                    |       4 | Writer identity and terminal recovery      |
| `src/ts/server/bootstrap.test.ts`                              |       5 | Runtime bootstrap adapter                  |
| `src/ts/server/bridgeFlush.test.ts`                            |       2 | Lifecycle bridge flush                     |
| `src/ts/server/characterShellHydration.test.ts`                |       4 | Character shell hydration                  |
| `src/ts/server/chatMessageHydration.reactivity.svelte.test.ts` |       2 | Hydration loading reactivity               |
| `src/ts/server/chatMessageHydration.test.ts`                   |      50 | Chat and lorebook hydration                |
| `src/ts/server/commands.test.ts`                               |     148 | Command transport and local effects        |
| `src/ts/server/durableMutationDispatch.test.ts`                |      18 | Durable live dispatch                      |
| `src/ts/server/durableMutationTerminalRejection.test.ts`       |       6 | Terminal/retryable dispositions            |
| `src/ts/server/events.test.ts`                                 |       7 | Command and memory SSE client              |
| `src/ts/server/hydrationReads.test.ts`                         |      12 | Body hydration wire clients                |
| `src/ts/server/pendingBridgeFlushRegistry.test.ts`             |       3 | Owner flush registry                       |
| `src/ts/server/pendingMutationOutbox.test.ts`                  |     191 | Encrypted outbox and durable route policy  |
| `src/ts/server/pendingMutationReplay.test.ts`                  |       6 | Bootstrap replay                           |
| `src/ts/server/promptTemplateHydration.test.ts`                |      20 | Prompt-owner hydration                     |
| `src/ts/server/resourceCache.test.ts`                          |       8 | Authenticated hash cache                   |
| `src/ts/server/resourceInvalidation.test.ts`                   |      90 | Event read planning and apply              |
| `src/ts/server/resourceReads.test.ts`                          |      14 | Root and targeted reads                    |
| `src/ts/server/resourceRefresh.test.ts`                        |       8 | Complete/Realm refresh                     |
| `src/ts/server/resourceState.svelte.test.ts`                   |      57 | Resource slices and acknowledgement fences |
| `src/ts/server/resourceWriteGuard.test.ts`                     |       8 | Guarded compatibility writes               |
| `src/ts/server/settingsDraftAcknowledgement.test.ts`           |       5 | Draft receipt settlement                   |
| `src/ts/server/settingsGroups.test.ts`                         |       4 | Settings ownership map                     |
| `src/ts/server/staleStateGuards.test.ts`                       |      14 | Shared stale-state/rollback guards         |
| `src/ts/storage/database.resourceState.test.ts`                |       3 | Database facade over resource state        |
| **Total**                                                      | **829** | **26 primary files**                       |
