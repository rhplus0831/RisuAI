...
- Highest-risk remaining gap: Phase 7 still needs stop-trigger mutation
  payload delivery and route-backed fixture coverage before chat-path
  closeout. Browser regenerate request wiring landed in Slice 7A,
  server regenerate assembly semantics landed in Slice 7B, and `/chat`
  provider dispatch guards landed in Slice 7C.
...
- Next default pickup: Phase 7 Slice 7D, stop-trigger mutation payload
  delivery.
