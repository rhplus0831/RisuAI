...
_Audited 2026-06-03. Scope: every command route in `server/fastify/src/routes/commands.ts`
(79 routes), against the reference fix `b57df5cd` ("fix: speed up character selection
command"). Findings were produced by a fan-out classifier pass and then individually
re-checked by an adversarial verifier that tried to refute each narrowing claim._

## What "mutation-range mismatch" means

A command logically changes a small slice of state (one settings scalar, one row of one
collection, one character row, one chat row) but is routed through a **broad mutation
helper** that physically rewrites far more — or its event maps to a **broad projection
resource** that re-ships whole arrays on refresh. `b57df5cd` is the canonical fix:
`characters/select` previously rewrote **every** character row + **all 9** collection
tables + the settings row to change one `currentChar` pointer and one character's
`lastInteraction`. It was replaced by `applyCharacterSelectionCommandMutation` /
`writeCharacterSelectionRows`, which write exactly **one character row + the settings
row** (`mutationPath: 'targeted-character-selection'`, `dbJsonWriteMs: 0` review gate),
and paired with a narrow `characterSelection` projection resource
(`loadCharacterSelectionProjection`, `RESOURCE_PROJECTION_FIELDS.characterSelection = []`).

## The shared mechanism

The JSON `database` is split across SQLite tables (`server/fastify/src/repository.ts`):
...
| --- | --- | --- |
| `applyJsonCommandMutation` | `hydrated` | loads **ALL** chat messages, then `syncChatMessages` (surgical) **+ rewrites ALL characters + ALL chats + ALL 9 collection tables + settings** |
| `applyMessageFreeJsonCommandMutation` | `message-free` | no message load, but **rewrites ALL characters + ALL chats + ALL 9 collection tables + settings** |
| `applyTargetedCommandMutation` | custom | loads message-free db.json for **validation only**; callback does its own targeted SQLite writes; runs the broad `replaceAll*` only if `writeDatabase: true` (default off). **The targeted path.** |
| `applyCharacterSelectionCommandMutation` | `targeted-character-selection` | bespoke: writes exactly one character row + settings row. **The reference fix.** |
...
So **71 of 79 commands share the exact shape `b57df5cd` fixed.** Post-verification
severity: 51 high, 18 medium, 3 low (write-amplification × hotness; "high" = rewrites
every character row and/or loads every message for a 1-row logical change).

## Cross-cutting prerequisites (apply to almost every finding)

These were surfaced by the adversarial verifier and must be respected; they keep the
fixes faithful rather than "looks narrow but drops data."

1. **The targeted writers do not exist yet.** `repository.ts` has only
   `writeCharacterSelectionRows` (one char row + settings) plus the broad `replaceAll*`
   and the surgical message-store writers. Realizing these findings requires building a
   small writer kit, each used via `applyTargetedCommandMutation` (`writeDatabase` unset)
   or a bespoke mutation like the reference:
   ...
   - `writeSingleCollectionTable(db, field, array)` — `DELETE`+reinsert one of the 9
     tables (for create/delete/reorder); and/or `writeSingleCollectionRow(db, field, position, value)` (for pure field edits).
   - `writePluginStorageKey` / `deletePluginStorageKey`.

2. **Whole-collection normalization is a deliberate dropped write.** Many `mutate`
   callbacks open with a repair pass (`ensureCharacterCollection`,
   `normalizeAllCharacterChats`, `ensureModuleCollection`/`ensureModuleRecords`,
   `ensureLorebookDatabase`→`ensureAllChildLorebooks`, `ensure*Collection`) that mutates
   **sibling/other rows in place** (re-IDs duplicate ids via `randomUUID()`, default-fills,
   clamps pointers). The broad path opportunistically persists those repairs for every
   row; a targeted write computes them and then discards them. This is the **same tradeoff
   `b57df5cd` already accepted** — `loadCharacterSelectionRows` reads one raw row and skips
   normalization. It is data-safe here precisely because of the project posture (no users,
   no migrations, dirty-backup data acceptable; see `CLAUDE.md`). Each targeted path should
   still re-normalize **its own** target row, and treat global de-dup as validate-only.

3. **"Single-X" usually means "one row **+ the settings row**."** Normalization commonly
   writes a settings scalar alongside the row edit (`characterOrder`/`currentChar` on
   character create/trash; `lastLoadedLoadoutName` on loadout edits; the collection's
   pointer clamp on preset/persona/translator edits). The targeted writer must conditionally
   `UPDATE settings` when that scalar actually changed — exactly as `writeCharacterSelectionRows`
   writes both the character row and settings.

4. **The universal cheap floor: drop `hydrated` → `message-free`.** 64 of the 66
   `applyJsonCommandMutation` routes never read or write `chat.message[]` (all but
   `characters/:id/chats` create and `chats/:id/fork`), so
   `loadPersistedWithMessages` + `syncChatMessages` are pure waste (the sync is a guaranteed
   no-op). Swapping those routes to `applyMessageFreeJsonCommandMutation` is a mechanical,
   zero-new-helper, zero-correctness-risk change that removes the all-messages load and the
   chat-row rewrite cost immediately. It does **not** reach the per-row target (it still
   rewrites all characters + 9 collection tables + settings), so it is a stopgap, not the
   fix — but it is the safe first commit for every `hydrated` non-message route below.

---

## Prioritized findings

Priority = write-amplification × call frequency × fix cleanliness. Each entry lists the
**actual** write range (what the helper physically writes) and the **desired** range (the
minimal rows that logically changed), already corrected for prerequisites 2–3.

### Tier 1 — Pure settings/pointer writes that rewrite the entire DB (highest ratio, cleanest fix)

These change **one settings scalar** but rewrite every character row + every chat row +
all 9 collection tables (and most also load every message). Closest to the reference fix;
the write narrows to a single `UPDATE settings`.
...
Homogeneous family. Each edits one element/ordering of **one** collection but rewrites all
9 collection tables + all characters + (on `hydrated`) loads all messages. Pure field edits
(`PATCH`, `enable`, `favorite`) can be single-row `UPDATE … WHERE position=?`; create/delete/
reorder shift positions and justify a full rewrite of **that one table**. A pointer scalar
(`botPresetsId`, `selectedPersona`, `translatorPresetId`, `loreBookPage`, `lastLoadedLoadoutName`,
`currentPluginProvider`) usually rides along in settings (prerequisite 3).
...
For these, the cheap floor (prerequisite 4) is also the safe ceiling: a true per-row write
is blocked by a genuine cross-table span or a load-bearing message/normalization dependency.
Apply `applyMessageFreeJsonCommandMutation` (or keep `hydrated` where noted) and stop.
...
`RESOURCE_PROJECTION_FIELDS` (`routes/projection.ts:34`) maps each command-event `resource`
to the top-level keys a **foreign/recovery refresh** re-ships. Under the single-writer
invariant these refreshes are rare (only server-originated events + gap recovery), so this
is secondary to the write-side findings — but narrowing a mutation without narrowing its
projection leaves the refresh broad, and the reference fix narrowed **both** (it added the
`characterSelection` resource → `[]` + a bespoke branch).

**Broad projection resources that re-ship whole arrays** (worth a bespoke narrow branch
when the matching mutation is narrowed):
...
**Pre-existing projection-field bugs** (the resource ships the *wrong* field — already
broken today, independent of the write range, but should be fixed alongside):
...
1. **Mechanical floor (one commit, no new helpers):** swap every `hydrated` non-message route
   to `applyMessageFreeJsonCommandMutation`. Removes the all-messages load + chat-row rewrite
   across ~62 routes with zero correctness risk. (Skip the genuinely message-dependent ones:
   2390, 2495, 2617, 2655, and the message commands.)
2. **Build the writer kit** (prerequisite 1) + add `commandMetrics.test.ts` review gates
   (`dbJsonWriteMs: 0`, like `targeted-character-selection`) for each new `mutationPath`.
3. **Tier 1 + Tier 2** — settings-only and plugin-storage routes: highest amplification,
   cleanest, projection already safe or sprawling-by-design. Model each on
   `applyCharacterSelectionCommandMutation`.
4. **Tier 3** — character/chat single-row routes; add the matching narrow projection branches
   (`characterSelection`/`characterLorebook` are the templates).
5. **Tier 4** — collection-table families, plugins first (projection already narrow), then the
   rest with their pointer-settings co-writes; fix the projection-field bugs in the same slices.
6. **Tier 5** — only after the normalization passes are scoped to validate-only; until then,
   leave at the `message-free` floor.

For every targeted path: re-normalize the **target** row, treat global de-dup as validate-only
(prerequisite 2), and add a regression test asserting unrelated rows are not rewritten (the
reference fix's `tableRowidsById` rowid-stability assertion in
`server/fastify/__tests__/commands.test.ts` is the template).
