...
- Active slice: [Memory-embedding configuration seam](phases/slices/phase-4-server-consumer-migration/memory-embedding-configuration-seam.md), ready.
...
- Runtime changes through Phase 1: shell, character-summary,
  ...
  wire changes.
- Latest implementation candidate: BardWiki server type seam at `44e53527a`,
  after the Phase 3 close at `96e0dedfb`; focused ownership and BardWiki
  canonical/event/prompt/apply/rebuild owners,
...
- 286 direct root-`src` edges remain: 187 production, 91 server-test, and 8
  browser-smoke, spanning 123 importers and 53 targets.
- Usage is 97 runtime, 38 mixed, and 151 type-only; 135 runtime/mixed edges
  remain.
- The completed Phase 1 and Phase 3 slices, plus the reviewed Workstream 2
  shared-helper reuse and Phase 4 BardWiki seam, removed 89 edges and 26 source
  targets.
...
| [4. Server consumer migration](phases/phase-4-server-consumer-migration.md) | Active | BardWiki type seam complete; memory-embedding configuration is next. |
...
Use [`next-steps.md`](next-steps.md). Give memory-embedding resolution and job
configuration narrow Fastify-owned input records while preserving model aliases,
credential selection, local-model rejection, provider bounds, and job behavior.
