...
provider-options slice is complete. The broader Phase 3 generation dispatch
phase remains in progress because other retained browser-local provider helper
branches still reconstruct provider options from flat fields; see
...
- Current verification state: browser request helper role/static/fallback
  ...
  tests pass, server-intent completion payload tests pass, the requested
  focused provider tests pass, client-lib TypeScript passes, full server strict
  TypeScript passes, and `git diff --check` passes. See
  [`latest-verification.md`](latest-verification.md).
...
1. Continue Phase 3 by broadening resolver-derived provider option use in the
   ...
   OpenAI-compatible chat-completions, OpenAI Responses, legacy instruct, and
   Anthropic-family requests now use profile provider options.
