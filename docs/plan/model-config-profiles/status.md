...
because other retained browser-local provider helper branches still reconstruct
provider options from flat fields; see
...
1. Continue Phase 3 by broadening resolver-derived provider option use in the
   ...
   and OpenAI-compatible chat-completions requests now use profile provider
   options.
