...
The browser-local native Ollama provider-options slice is complete. The
browser-local Cohere/Horde/Ooba legacy provider-options slice is complete.
Phase 3 generation dispatch is complete; Phase 4 is not started. See
[`latest-verification.md`](latest-verification.md).

## Snapshot

- Plan state: open.
- Current phase: Phase 3 generation dispatch complete; Phase 4 not started.
  Phase 3a complete;
  ...
  provider-options slice complete; browser-local native Ollama
  provider-options slice complete; browser-local Cohere/Horde/Ooba legacy
...
- Current implementation state: existing flattened `Database` fields remain the
  ...
  Browser-local `requestCohere()` now uses profile-owned request models, Cohere
  URLs, API keys, extra headers, and additional params when a resolved profile
  is present; profile model ids drive the newer Command R safety-mode decision,
  while no-resolved-profile callers keep legacy URL/key behavior.
  Browser-local `requestHorde()` now uses profile-owned request models and API
  keys, preserving anonymous `0000000000` behavior for missing or blank profile
  keys, and reads profile/runtime context, max response, temperature, top-k, and
  top-p values where those fields match the existing Stable Horde request body.
  Browser-local `requestOobaLegacy()` now uses profile-owned base URLs and API
  keys, normalizes profile URLs to `/api/v1/generate` and `/api/v1/stream`,
  fails with `options["ooba-legacy"].baseUrl is required` before network work
  when a resolved profile has no URL, omits `X-API-KEY` for blank profile keys,
  and reads profile runtime max response, max context/truncation, and
  temperature fields where they match the existing body.
  Outside those dispatch slices, durable profile storage, UI writes, provider
  secret reshaping, and auxiliary surfaces remain on the existing flat fields.
...
- Current verification state: browser request helper role/static/fallback
  ...
  provider-options tests pass, browser-local Cohere/Horde/Ooba legacy
...
1. Treat Phase 3 dispatch parity as complete unless validation exposes a
   regression. Do not start Phase 4 in this slice.
