...
measured families intentionally remained on the hydrated generic path until a
separate slice defined their safety rules.

Second follow-up result on 2026-06-01:
...
`pluginStorage.updated` now uses the message-free plugin-storage mutation path.
Chat, message, and generation families intentionally remain on the hydrated
generic path.
