...
Status: not started. Phase 7. Bundles regex/compile memoization, redundant DB
work, and logging cleanup. Output and behavior stay unchanged.
...
- M2 precondition: module/script-resolution inputs are invariant across the
  per-message loop; only `data`/`chatRole`/`chatID` vary.
