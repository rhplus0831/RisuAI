...
Server trigger safety and hot-path boundedness change.

## Scope

Bound user-supplied regular expression work in the server trigger
interpreter so one pathological pattern cannot wedge the event loop inside a
single synchronous RegExp operation.

This slice owns the server trigger data-effect helpers that compile or execute
user regexes and the error path surfaced back through `runTrigger`. It does
not change client-side trigger execution, Lua budgets, trigger step accounting
outside regex effects, or legitimate regex results below the selected caps.
...
- Route every user-supplied regex compilation/execution in server trigger data
  effects through one bounded helper, including condition-time regex checks
  and data-effect arms.
...
- Keep valid regex behavior unchanged below the caps: flags, captures,
  ...
  values should match the old implementation.
