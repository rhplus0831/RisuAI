...
| ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| Prompt construction whole-corpus work      | Server prompt assembly still loads a hydrated persisted database once per assembly through `loadPersistedWithMessages()`. Prior Phase 2 work narrowed assembly-time writes, but not the read/construction pass.  | [`generation-prompt-construction-pass-measurement.md`](phases/slices/phase-2-command-write-cost/generation-prompt-construction-pass-measurement.md)     | First              |
...
The best next work candidate is Phase 2 prompt-construction measurement. It is
the hottest user-facing path among the remaining risks and already has opt-in
generation metrics, so the next step can be a focused metric extension before
