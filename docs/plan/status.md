...
- Plan state: open. Phases 1-3 and 5 are implemented and proof-refreshed;
  Phase 4 remains the next pending fix batch under the current sequencing.
...
- Scheduled IDs: H1-H3, Phase 2's M5, M6, L3, L13, L14, L16, K1, and K2,
  ...
  registered `DONE` in the v2 gate and `active-risk-analysis.md`. Remaining
  scheduled gate entries stay `PLANNED`; risk-map rows stay `PENDING`.
...
- Current Phase 5 proof: focused render/UI suites, render-count/script proof,
  parser companion suites, both gates, `pnpm test`, client-thinning audit, and
  both TypeScript checks passed. `pnpm check` retains the documented
  svelte-check baseline in [`latest-verification.md`](latest-verification.md).
...
- [Phase 4](phases/phase-4-client-clone-ring-2.md): next pending. Client clone
