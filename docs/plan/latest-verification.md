...
- Phase 7 M2/L3/L8/L9/L37-L40 (`151c6978`) — PHASE 7 COMPLETE (the last
  scheduled batch): a simulated 25-message history window compiles each
  regex script **once** instead of once per message (counting-RegExp proof;
  outputs byte-identical to the per-call cold baseline; cbs-action scripts
  keep their per-message compile by design; a replaced script list
  invalidates the memo). One lorebook activation compiles each
  `/pattern/flags` key **once** across 8 messages × multiple recursive
  passes × 3 entries (was per message × per key × per pass); malformed keys
  keep their deactivate-without-throw parity. A trigger pass using the same
  pattern in two effects compiles it **once per pass** (was once per
  effect; the pass-end `ReloadGUIPointer` reset bounds the memo lifetime
  for var-writing passes, proven in the same test). Command-event pruning
  issues **one keep-window range DELETE** per write (`revision <= latest -
  limit`) instead of an `ORDER BY ... LIMIT 1 OFFSET 999` walk; contiguous
  retention is unchanged and the keep-window semantics are pinned. A
  character delete issues **no explicit `chats` DELETE** — the FK cascade
...
| 2026-06-05 | Phase 7 memoization-and-hygiene batch (`151c6978` fix + gate/doc flip) — PHASE 7 COMPLETE; ALL SCHEDULED PHASES DONE | `pnpm api:test` 1737/1 (99 files; +8 over the post-audit-closeout 1729 baseline: `scripts.test.ts` +4 M2, `lorebook.test.ts` +2 L3, `events.test.ts` +1 L8, `repositoryWriterKit.test.ts` +1 L9), `pnpm test` 1130/4 (121 files; +8: `command.projectionGuard.test.ts` +1 L37, `scripts.editdisplay.test.ts` +1 L38, `serverBackedSendChat.findMessage.test.ts` +3 L39, `triggers.regexMemo.test.ts` +3 L40), audit green, both tsc checks zero errors. History window: 1 regex compile per script (was 1 per message × script); lorebook activation: 1 compile per key (was per message × key × pass); trigger pass: 1 compile per pattern (was per effect); event prune: 1 range DELETE (was OFFSET-999 walk per write); char delete: FK cascade only, byte/metric parity; warm/render paths log nothing; terminal lookup copies nothing.                                                                                                                                                                                                                                                                                                                                                                              |
| 2026-06-05 | Phase 8 closing full run (doc-only flip; no runtime change) — PHASE 8 COMPLETE; PLAN CLOSED                          | `pnpm test` 1132/4 (122 files; +2 tests / +1 file over the Phase 7 row from the audit-closeout's `database.importPreset.test.ts`, registered as L37 `extraTests`), `pnpm api:test` 1737/1 (99 files, carried), `pnpm client-thinning:audit` green, both tsc checks zero errors. The fix-completeness gate registry holds every scheduled id as `DONE` with zero `PLANNED` entries; gated (L4, L7, L26, U2), no-action (U3), and dismissed (R1-R5) lists unchanged. The gate test stays live as the standing maintenance check; re-run this proof set and add a row after any future change to a narrowed/bounded path. |
