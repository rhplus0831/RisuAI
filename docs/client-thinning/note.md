...
- Audit fixture reproducibility is the first standalone open item. Every A4R
  ...
  drift`, `EC5 active-writer guard`, `EC4 stable command ids`, and `EC2 plugin
  storage gates` now have committed pre-fix fixtures and tests proving non-zero
  exit. The remaining open rules are the EC/AEC structural invariants (`EC1`,
  `AEC2`, `AEC4`, `AEC5`, `AEC6`), which still need committed fixtures.
...
2. Continue audit fixture reproducibility unless source inventory proves a more
   urgent live bug. All A4R rules, `EC6`, `EC5`, `EC4`, and `EC2` are covered;
   the next fixture target is `EC1 provider ownership`, then the remaining AEC
   structural rules (`AEC2`, `AEC4`, `AEC5`, `AEC6`).
