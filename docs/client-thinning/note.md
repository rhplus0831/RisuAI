...
- Audit fixture reproducibility is the first standalone open item. Every A4R
  rule (`A4R1`–`A4R7` plus the `A4R-` named rules) and all EC rules (`EC1`,
  `EC2`, `EC4`, `EC5`, `EC6`) now have committed pre-fix fixtures and tests
  proving non-zero exit. The remaining open rules are the AEC structural
  invariants (`AEC2`, `AEC4`, `AEC5`, `AEC6`), which still need committed
  fixtures.
...
2. Continue audit fixture reproducibility unless source inventory proves a more
   urgent live bug. All A4R and EC rules are covered; the next fixture target is
   `AEC4 chat folder identity scope`, then the remaining AEC structural rules
   (`AEC2`, `AEC5`, `AEC6`).
