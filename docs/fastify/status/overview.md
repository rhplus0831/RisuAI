...
| 6 - Server-side generation              | in progress | Completion route plus echo/OpenAI/NanoGPT/OpenRouter/Anthropic. |
...
- **Server.** Captured in [`server.md`](server.md). Phases 1 +
  2 + 3 are landed; Phase 6 has started with
  `/api/v1/generate/completion`. Docker targets Fastify and Express
  has been deleted. The production image installs only production
...
- **sendChat.** Captured in [`sendchat.md`](sendchat.md). Phase 5
  ...
  extracted into focused modules. The current guardrail is 29 local
  snapshots plus 3 server-backed provider-parity fixtures; the
