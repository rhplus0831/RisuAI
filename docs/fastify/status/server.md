...
landed 32 slices through 7-10c: the chat-route scaffold,
...
`runStartTrigger` handoff, and the template renderer through content +
chat cards:
...
- **Phase 7.** Server-side prompt assembly + lorebook activation.
  In progress. Thirty-two slices have landed through 7-10c:
  ...
  `runStartTrigger` handoff, and the template renderer through content +
  chat cards. The trigger and history fronts are complete; next slice
  is 7-10d, memory cards + cache markers.
