...
Phase 7 is active with thirty-eight slices landed through 7-11c:
...
the 7-11b static/plain slot fill (`fillStaticSlots`), and the 7-11c
lorebook placement + preflight (`fillLorebookSlots`, with
`buildLorebookContext` in `lorebook.ts`), but still throws past scope
...
1. **Continue Phase 7 with slice 7-11d — history window + bias rows.**
   7-11c (`34e820d9`) added lorebook placement + preflight. 7-11d runs
   the async history window and collects the bias rows. SPA reference is
   `src/ts/process/index.svelte.ts:227-241` (history) and `:265-273`
   (bias).

   Verified slice scope:
   - add an async `fillHistoryAndBias(state)` calling
     `buildHistoryWindow(ctx, currentChar, currentChat,
usingPromptTemplate, NO_ASSETS, state.report)`: on
     `result.stopSending`, set `state.stopSending`; else
     `currentTokens += result.addedTokens`,
     `currentChat = result.currentChat`,
     `triggerResult = result.triggerResult`, and stash
     `result.messages` for the 7-11e memory window — **do not** push them
     into `unformated.chats` here.
   - bias rows (`:265-273`): `db.bias.concat(currentChar.bias)` mapped
     through `expandVariables(unescape(k), { …ctx, chara })` (unescape
     `\\n` / `\\r` / `\\\\`), keeping the `[string, number]` weight.
   - extend `AssemblyState` with `historyMessages` / `triggerResult` /
     `stopSending` / `biases`.
   - Boundary: the SPA pushes history into `unformated.chats` via the
     **memory window** (`:243-263`), so the `chats` fill + `lastChat`
     promotion + depth-prompt splice + `additonalSysPrompt` placement are
     all **7-11e**, not here. Inlay/multimodal asset lookup stays
     browser-side (`NO_ASSETS`).
   - Follow-ups are 7-11e memory/post-history slot mutations, 7-11f final
     render + budgeted prompt payload, 7-11g chat route wiring, 7-11h
     preview shortcut, and 7-11i telemetry. Hypa V3 stays Phase 8;
     browser plugin/Lua stays deferred.
   ...
   tests green. Last recorded baselines are `pnpm api:test`: 853
