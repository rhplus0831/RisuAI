...
`src/ts/process/index.svelte.ts` is 662 lines. Phase 5-1 through
5-23 already extracted auto-continue, owned `doingChat` lifecycle,
...
`<Previous Conversation>` wrapping), and the final prompt render
...
| 413-494 | Budget recheck, generation metadata, preview exits, provider request payload, fail / abort exits.                             | dispatch helper                      |
| 495-596 | Stream vs non-stream orchestration, output-trigger result, inlay / TTS side effects, auto-continue and IGP.                   | response orchestration helper        |
| 601-654 | Resend handoff, notifications, emotion fallback routing, image generation dispatch, final timing writeback.                   | stage-4 orchestrator                 |
