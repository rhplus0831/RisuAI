...
| `src/ts/characters.ts`, `characterCards.ts`, `PlaygroundMenu.svelte`, catalog UI                    | Character create/import/update/delete/order, images, chat import, cold-storage hydration                       | Character catalog/profile commands; server-backed `.risu` imports use the 9-8 route          | Tauri/local card import remains local                                                | High: large object replacement and imports                  | 9-3a, 9-8           |
...
| `src/ts/process/lorebook.svelte.ts`, `LoreBookList.svelte`, `lorepreset.svelte`                     | Global, character, chat, and module lore entries/settings                                                      | Lorebook and child replacement commands; full-save import/export uses the 9-8 `.risu` routes | Tauri/local lorebook import/export remains local                                      | Medium: nested child arrays                                 | 9-4a                |
...
| `src/ts/storage/risuSave.ts`, `backup.ts`, `kei/backup.ts`                                          | `.risu` decode/encode, restore, local/remote backup save, asset cache walking                                  | Server `.risu` codec, import/export routes, bundle export, and backup restore events landed  | Tauri and local backups remain local                                                 | High: whole-state replacement and assets                    | 9-6, 9-7, 9-8       |
