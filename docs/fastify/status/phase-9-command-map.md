...
| Family | Endpoints | Payload notes | Event names | Slice |
| --- | --- | --- | --- | --- |
| Settings groups | `PATCH /api/v1/commands/settings/:group` | Groups: `providers`, `runtime`, `prompt`, `display`, `language`, `media`, `memory`, `advanced`, `sidebar`, `account`. Server validates an allowlist per group. Provider key placeholder strings mean "leave unchanged" after bootstrap masking lands. | `settings.updated` | 9-2a |
...
- Add the shared Fastify command route plumbing and repository mutation
  helper against the current `db.json` blob.
- Add a simple in-process command event sink/catalog that 9-5 can expose
  over SSE later.
- Add `src/ts/server/commands.ts` with the typed browser command fetch
  helper.
- Ship one small `PATCH /api/v1/commands/settings/runtime` allowlisted
  setting as the harness command.
- Do not replace broad UI mutation call sites until 9-2 and later.
