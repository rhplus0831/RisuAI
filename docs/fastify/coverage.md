...
- Phase 9 coverage has landed for command routes through the 9-4 resource
  ...
  storage and provider-secret gates, 9-7/9-8 server `.risu` codec,
  import, export, asset-report, and bundle behavior, and 9-9 integrated
  Fastify-served browser smoke/storage-write coverage. Server command routes
  ...
  cold-storage, and MCP credential gates in their focused tests; `.risu`
  behavior in the `risuSave*` Fastify tests; and the integrated browser path
  in `server/fastify/browser-smoke/fastifyBrowserSmoke.spec.ts`.
