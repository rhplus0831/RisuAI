...
runtime surfaces (alternative server adapters, legacy local mode/mobile wrappers, service
...
The legacy local client was explicitly removed in commit `a8dd411c`
(2026-05-27). All `@tauri-apps/*` dependencies, legacy local mode config, and
legacy-local-mode-specific code paths are deleted. The web client only runs
