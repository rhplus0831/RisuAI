...
Status: in-progress (47 slices landed as of 2026-05-24).
...
template renderer is **complete**, and `assemble.ts` now holds the full
7-11a–f assembler: the 7-11a state/context loader (`beginAssembly`), the
7-11b static/plain slot fill (`fillStaticSlots`), the 7-11c lorebook
placement + preflight (`fillLorebookSlots`, with `buildLorebookContext`
in `lorebook.ts`), the 7-11d history window + bias rows
...
`/chat` and `/preview-prompt`) and the 7-12c preview-path wiring (`sendChat`
routes `preview` / `previewPrompt` through `/chat` behind the gate). The
**send** path is not yet wired — it is dispatch-coupled (needs
`message_patch` chat-row deltas) and lands with 7-12d. The remaining
Phase 7 work is the rest of the Tier 4 browser adapter (7-12d onward). See
...
| 7-12a   | `a3a1e6e2` | Browser client adapter `requestServerChat` (`serverChat.ts`): POSTs `AssembleInput` to `/chat` with `risu-auth`, stream-parses `stage`/`prompt`/`info`/`error`/`done` (ignores dispatch-coupled events), returns `{ status }`. Shared `sseParse.ts` (lifted from `serverCompletion.ts`); introduced the `db.useServerPromptAssembly` gate later used by 7-12c. |
...
| 7-12c   | `8cf7fd63` | Wired the `sendChat` **preview** paths through `/chat` behind the gate: `preview` → `previewFormated`, `previewPrompt` → `previewBody`, `error`→`throwError`, `aborted`→false. Send/continue/regenerate stay local (7-12d). Added `sendChat.serverPreview.test.ts`. Cross-assembler parity test deferred (needs a normalized-DB bridge).                       |
...
full 7-11a–f assembler (`beginAssembly` + `fillStaticSlots` +
`fillLorebookSlots` + `fillHistoryAndBias` + `fillMemoryAndPostHistory` +
`renderAndBudget`); `assemblePrompt` chains them and returns the payload
(the critical-path assembler is closed, route wiring is 7-11g);
...
- **7-10d** — Memory cards + cache markers. **Landed `3983d2d0`**
  (10 added tests). Renders `memory` cards from the injected
  `memories[]` (clone + `innerFormat` `{{slot}}` wrap, no
  positionParser), applies explicit `cache` cards (walk back `depth`
  rows whose role matches; `all` matches any), and applies the
  automatic 3-deep `user` cache-point walk-back when
  `db.automaticCachePoint` is set and no cache card suppresses it — all
  in `renderByTemplate`. `preflight.ts` unchanged (still flags
  `memoryCardUsed` / `hasCachePoint` only).
- **7-10e** — Position + prompt-info finalization. **Landed
  `2871960f`** (9 added tests). `renderByTemplate` now returns
  `{ formated, promptInfo }`: with both `promptInfoInsideChat` and
  `promptTextInfoInsideChat` on, it collects a parallel info array in
  lockstep with card rendering via a `deps.promptInfo` sink (raw
  `innerFormat` for persona / description / authornote / memory; parsed
  content for non-globalNote plain / jailbreak / cot, via the SPA's
  `pushPromptInfoBody` which skips empty `fmt`), and trims row contents
...
- **7-11a** — `assemble.ts` state/context loader + assembler contract.
  **Landed `e0902944`** (13 added tests). `beginAssembly` resolves the
  database / character / chat through the `AssembleDeps.loadDatabase`
  seam (id→index, `EntityNotFoundError` on a missing database or
  unmatched id), builds the `ExpandContext` + the
...
- **7-11c** — Lorebook placement + token preflight. **Landed
  `34e820d9`** (7 added tests). Ported `buildLorebookContext` into
  `lorebook.ts` (slot distribution `lorebook` / `description` /
  `postEverything` + `positionParser` + `depthPrompts`) and added
  `fillLorebookSlots`: `activateLorebook` → distribute →
  `preflightTemplateTokens`, seeding `currentTokens = maxResponse + 50 +
preflight.addedTokens` plus `report` / `positionParser` /
  `depthPrompts` / `memoryCardUsed` / `hasCachePoint`. No history,
  memory, or render.
- **7-11d** — History window + bias rows. **Landed `3992b967`** (3
  added tests, api:test 853 → 856). `fillHistoryAndBias(state)` (async)
  runs `buildHistoryWindow(ctx, currentChar, currentChat,
usingPromptTemplate, NO_ASSETS, state.report)`, threads the
  start-trigger mutations (`currentChat` / `triggerResult` /
  `varChanged`), honors `stopSending` (short-circuit before token/bias
  capture), folds `addedTokens` into `currentTokens`, captures
  `historyMessages`, and collects the `biases` (`db.bias ∪ char.bias`,
  unescape `\n` / `\r` / `\\`, then `expandVariables(…, { chara })`,
  weights preserved). `NO_ASSETS` exported from `history.ts`. History
...
- **7-11e** — Memory bridge + post-history slot mutations. **Landed
  ...
  budget pruning.
- **7-11f** — Final render + budgeted prompt payload. **Landed
  `3492bede`** (6 added tests, api:test 868 → 873). Added
  `renderAndBudget(state)`: `renderFinalPrompt` over the complete slots
  (`state.formatOrder` already has `postEverything` appended, so it is
  not re-pushed; `editRequest` keeps its identity default) then
  `finalizeRequestBudget`, aborting with `abortReason: 'overflow'` when
  the pinned rows don't fit, else stashing `formated` / `inputTokens` /
  `outputTokens` / `promptText`. `assemblePrompt` now chains 7-11a–f and
  returns the widened `AssembleResult` (`stopSending`, `abortReason?`,
  `prompt?` = the SSE payload, plus `formated` / `biases` /
...
- **7-12a** — Client adapter for `/api/v1/generate/chat`. **Landed
  `a3a1e6e2`** (13 added SPA tests, test 601 → 614). `requestServerChat`
  (`serverChat.ts`) POSTs the `AssembleInput` body with the `risu-auth`
  header and stream-parses the named SSE events (`stage` / `prompt` /
  `info` / `error` / `done`), ignoring the dispatch-coupled `token` /
  `message_patch` / `side_effect` / `warning` events, and returns
  `{ status: 'ok', prompt, info } | { status: 'error', error } |
{ status: 'aborted' }`. The `event:`/`data:` frame parser was lifted
  into the shared `sseParse.ts` (`parseSseEvent` + `iterateSseEvents`,
  no behavior change to `serverCompletion.ts`); `serverChatEvents.ts`
  mirrors the locked `PromptChatEvent` union client-side. Added the
  `db.useServerPromptAssembly` gate (default false, orthogonal to
  `useServerGeneration`); 7-12c wires that gate for preview paths while
  send/continue/regenerate wait for 7-12d. Read-only: standalone adapter
  - `serverChatFetch.ts` mock + unit tests, no server change.
...
- **7-12c** — Preview-path `sendChat` wiring. **Landed `8cf7fd63`**
  (3 added SPA tests, test 615 → 618). Gated the `sendChat` **preview
  path** (`preview` / `previewPrompt`) on `useServerPromptAssembly`: a
  short-circuit right after `setupSendChatContext` calls `requestServerChat`
  and fills `previewBody` from `promptInfo.promptText` (`previewPrompt`) or
  `previewFormated` from the 7-12b `formated` (`preview`); `error` →
  `throwError`, `aborted` → false. The preview path needs no dispatch and
  no chat-row deltas, so it is wireable read-only. The send path stays
  local (7-12d). Added `sendChat.serverPreview.test.ts`. **The planned
...
- **7-12d** — Live **send-path** wiring + provider dispatch + the full
  ...
  error/abort restoration. **Re-sliced 2026-05-24** after a scope re-check
  found the old single "dispatch cluster" hid four distinct surfaces with
  independent producers, LOC envelopes, and rollback risks. Land in order:
  - **7-12d-i** — server→browser mutation handoff contract +
    `varChanged` persistence. Capture in `AssembleResult` what the
    server mutates that the browser used to mutate locally before
    `dispatchRequest`: start-trigger chat edits (`cutchat` /
    `modifychat` / `impersonate` on `currentChat.message`), `setvar`
    `chatVars` deltas, the `additonalSysPrompt` rows, the user-message
    row that `sendChat` used to push pre-dispatch. Wire `varChanged`
    persistence through the route (`generationChat.ts:144` already
    flags this as 7-12 work). No new SSE event — the contract is a
    typed `AssembleResult` extension so 7-12d-ii can serialize it.
    Browser stays read-only on this slice. Small–medium (~300–500 LOC).
  - **7-12d-ii** — `message_patch` SSE event + SPA applier. Define
    `MessagePatchEvent.patch` (insertion / update / replacement; index;
    role; data fields). Emit it from `/chat` for the 7-12d-i contract.
    `serverChat.ts` stops ignoring the event; a `sendChat` applier
    writes the patches into `currentChat.message` before
    `dispatchRequest`. The send path **still runs local dispatch** in
  ...
  - **7-12d-iv** — `side_effect` event + `error.restoration` rollback.
    ...
    the existing `sayTTS` path. `image` / `stable_diff` have **no
    Phase 7 producer** (image-gen is later); `hypav3_progress` is
    **Phase 8**; the SPA consumer should accept these as no-ops.
    Populate `ErrorEvent.restoration` with the pre-dispatch chat
    snapshot needed to roll back the 7-12d-ii patches on dispatch
...
- **Follow-up (parallel, not blocking) — cross-assembler dual-mode
  ...
  specifiers do not resolve under SPA vite). Needs a **normalized-DB parity
...
`phase-7-prompt-assembly.md` with the closeout summary and move the
landed detail into this archive. Flip `status.md` and
`status/next-steps.md` to Phase 8 (memory) as the next phase. The three providers
