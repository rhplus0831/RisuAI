...
Status: in-progress (9 slices landed as of 2026-05-23). `variables.ts`,
...
(regex-only `processScript` — see 7-6a notes for the skip list) are
real. The remaining assembly modules under `server/fastify/src/prompt/`
...
- **7-6b** — Special action prefixes: `@@move_top`,
  ...
  emotion-image side effect is browser-only). Next pickup.
- **7-6c/d/e** — `ableFlag` `<order, actions>` DSL, script-cache,
  module regex scripts, `runTrigger('display', …)` for
