...
Status: in-progress (8 slices landed as of 2026-05-23). `variables.ts`,
`staticSections.ts`, `plainSections.ts`, `history.ts` (minimal walk
only — see 7-5a notes for the skip list), and `scripts.ts` (regex-only
`processScript` — see 7-6a notes for the skip list) are real. The
...
- **7-6b/c/d/e** — Special action prefixes (`@@emo`, `@@move_top`,
  `@@move_bottom`, `@@inject`, `@@repeat_back`), the
  `ableFlag` `<order, actions>` DSL, script-cache, module regex
  scripts, `runTrigger('display', …)` for `editdisplay`. Order
  TBD per slice.
