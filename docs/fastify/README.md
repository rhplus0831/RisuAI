...
Current status lives in [`status.md`](status.md). Phases 0-9 were
closed for the Fastify-served web migration scope in `edbc2d07`. The
first audit follow-up and the second-pass alpha audit are both closed;
the alpha broad closeout landed in `50d55b97`. Fastify is the live
server path, Express has been deleted, and the Dockerfile / compose file
target port 6002 with `/app/data` persisted.
