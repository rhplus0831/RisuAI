...
- Last landed slice: 9-3a, Character catalog and scalar profile.
- Current gap: 9-3b continues chat record/folder commands; 9-5 should
  still do the residual direct-write sweep before enabling the read-only
  `DBState.db` guard.
- Next default pickup: 9-3b, Chat records, folders, and metadata.
- Last recorded full baselines after 9-3a: `pnpm check` clean,
  `pnpm test` 678 tests plus 4 skipped, `pnpm api:test` 1084 tests, and
...
| Client thinning                             | Active; 9-3a character catalog/scalar profile landed; next slice is 9-3b chat records/folders. |
