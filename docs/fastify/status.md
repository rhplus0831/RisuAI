...
- Last landed slice: 9-3e, Chat `scriptstate` and scripting side effects.
- Current gap: 9-3f replaces compatibility setters and access adapters that
  can still bypass resource commands; 9-5 should still do the residual
  direct-write sweep before enabling the read-only `DBState.db` guard.
- Next default pickup: 9-3f, Compatibility setters and access adapters.
- Last recorded full baselines after 9-3e: `pnpm check` clean,
  `pnpm test` 682 tests plus 4 skipped, `pnpm api:test` 1097 tests, and
...
| Client thinning                             | Active; 9-3e chat scriptstate landed; next slice is 9-3f compatibility adapters. |
