...
Current default pickup: **7-12b** (live `sendChat` wiring + dual-mode
assembly sweep, Tier 4). Tier 3 is closed and **7-12a** (the standalone
browser client adapter `requestServerChat`, behind the
`db.useServerPromptAssembly` gate) is landed. The template renderer
...
telemetry (7-11i) are landed. `message_patch` is dispatch-coupled and
folds into Phase 7-12c/d.
...
- **7-11f** — final render + budgeted prompt payload. **Landed
  `3492bede`.** Added `renderAndBudget` (`renderFinalPrompt` over the
  complete slots — no `postEverything` re-append, identity `editRequest`
  — then `finalizeRequestBudget`, aborting with `abortReason: 'overflow'`
  when pinned rows don't fit) and unblocked `assemblePrompt`: it chains
  7-11a–f and returns the `AssembleResult` (the `prompt` SSE payload +
  `formated` / `biases` / `inputTokens` / `outputTokens`) or
  `{ stopSending, abortReason }`. The throw is gone; the critical-path
  assembler is closed. No route dispatch yet.
- **7-11g** — wire `POST /api/v1/generate/chat` to call `assemble.ts`
  and emit `prompt` + `done` SSE events. **Landed `89a80c19`.** The
  route takes `dataDir`, binds `loadDatabase` to
  `loadPersisted(dataDir).database`, maps the body → `AssembleInput`,
  and streams `stage(validate)` → `stage(prompt,start)` → `prompt` →
  `stage(prompt,end)` → `done`; `stopSending` or any thrown error
...
- **7-11i** — SSE telemetry: `info` event on the `/chat` stream.
  **Landed `807f5d1a`.** `streamAssembly` times `assemblePrompt` and
  emits `info` on the success path (after `prompt`/`stage(prompt,end)`,
  before `done`) with `timings.prompt`, `tokens.{prompt,total}` from
  `result.inputTokens`, and the clamped `result.outputTokens` on a new
  `InfoEvent.responseBudget` field (a budget, not a completion count).
  The error / `stopSending` path emits no `info`. `/preview-prompt` is
  unchanged (its JSON body already carries the counts via `promptInfo`).
  `message_patch` (chat-row deltas) is dispatch-coupled and folds into
  Phase 7-12, not this slice.

7-11a through 7-11f (the critical-path assembler), 7-11g (the `/chat`
route), 7-11h (the preview shortcut), and 7-11i (the `info` telemetry)
are all landed — **Tier 3 is closed**.
...
- **7-12a** — client adapter for `/api/v1/generate/chat`. **Landed
  `a3a1e6e2`.** `requestServerChat` (`serverChat.ts`) POSTs the
  `AssembleInput` body with `risu-auth` and stream-parses the named SSE
  events (`stage` / `prompt` / `info` / `error` / `done`), ignoring the
  dispatch-coupled `token` / `message_patch` / `side_effect` / `warning`
  events, and returns `{ status: 'ok' | 'error' | 'aborted' }`. The
  `event:`/`data:` frame parser was lifted into the shared `sseParse.ts`
  ...
  live integration is 7-12b. Read-only: standalone adapter + mocked unit
...
- **7-12b** — live `sendChat` wiring + dual-mode assembly sweep: gate the
  ...
  **Precondition:** the `prompt` SSE event must first **additively** emit
  ...
  `messages` + `promptInfo`), which `dispatchRequest` needs.
...
- The template renderer is closed (7-10a–f landed), the **critical-path
  ...
  (7-12a) is landed; next is the live `sendChat` wiring + assembly sweep
  (7-12b).
...
1. **7-12b** — live `sendChat` wiring + dual-mode assembly sweep
2. **7-12c** — side-effect dispatch
3. **7-12d** — error / abort restoration
4. **7-13** — phase 7 closeout
