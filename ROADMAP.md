...
Current default pickup: **7-12d-i** (server→browser mutation handoff,
first slice in the live send-path / dispatch cluster). Tier 3 is closed;
**7-12a** (the browser client adapter `requestServerChat`),
**7-12b** (the additive `prompt`-event `formated` + `biases`
extension), and **7-12c** (the preview-path
...
- **7-12a** — client adapter for `/api/v1/generate/chat`. **Landed
  ...
  `useServerGeneration`), which 7-12c now uses for preview paths while
  send/continue/regenerate wait for 7-12d. Read-only: standalone adapter
  and mocked unit tests, no server change.
- **7-12b** — additive `prompt`-event extension. **Landed `2b5603a2`.**
  ...
  `messages` stays the lossy `{ role, content }` projection. `assemblePrompt`
  folds both into `result.prompt` so `/chat` and `/preview-prompt` stay in
