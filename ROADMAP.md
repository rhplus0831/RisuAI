...
- **7-9d** — V2 control flow + safe data effects. Split on
  ...
  - **7-9d-ii** — V2 safe data helpers. **(next)** The remaining
    side-effect-free leaf arms: message readers, string / array /
    dict / math helpers, random, tokenize, regex test/extract, and
    quick chat search. Each is `resolve → compute → setVar`.
...
V2 control-flow core. The next default pickup is **7-9d-ii** (V2 safe
data helpers) → 7-9e/f, with **7-10a** → 7-10b/c/d/e/f for templates
as an equally valid parallel front.
...
- The biggest parallel-able fronts are now **7-9d-ii** (continues
  triggers; the next default, after 7-9a/b/c + 7-9d-i `1bd8313b`)
  and **7-10a** (kicks off templates). 7-5d is the remaining Tier 1
  sub-slice and unblocks the moment 7-9f lands.
...
1. **7-9d-ii** — V2 safe data helpers
