...
Current default pickup: **7-11d** (history window + bias rows). The
template renderer (7-10a–f) and the 7-11a/b/c assembler steps (loader,
static/plain fill, lorebook placement + preflight) are landed; the
assembler now adds the history window + bias rows.
...
- The template renderer is closed (7-10a–f landed) and the 7-11a/b/c
  loader + static/plain fill + lorebook placement are in. 7-11d through
...
1. **7-11d** — history window + bias rows
2. **7-11e** — memory bridge + post-history slot mutations
3. **7-11f** — final render + budgeted prompt payload
4. **7-11g** — wire `/api/v1/generate/chat`
5. **7-11h** — `/api/v1/generate/preview-prompt`
6. **7-11i** — SSE telemetry (`info`, `message_patch`)
