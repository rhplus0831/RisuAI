...
This is a broad stability/performance audit of the current Fastify-only RisuAI
codebase. It follows the deep-clone narrowing, server/client protocol, and lazy
projection workstreams, and focuses on gaps those efforts did not cover.

The code is the source of truth. Every finding below was located in current code
and independently re-verified. Line numbers will drift; use symbol names as the
durable anchors.

## Scope And Context

- Deployment model: single-user self-host. Crashes, data loss/corruption, and UI
  hangs are serious; tenant-only concerns are not.
- Not yet released, no real users. DB migrations are out of scope; normal-use
  data loss/corruption is still in scope.
- Prior workstreams are in `.archived-docs/performance-and-stability/frontend-performance`,
  `.archived-docs/protocol-and-persistence/server-client-protocol`, and
  `.archived-docs/protocol-and-persistence/lazy-projection`. This audit reports current-code gaps, not
  already-fixed issues.
- `.archived-docs/deferred-work/leftover.md` tracks deferred or evidence-gated follow-ups.
  Matching findings are tagged [known-leftover].

## Method

The audit covered server generation, SQLite command mutations,
projection/events, jobs/streams/timers, Hypa V3 memory, Lua/provider adapters,
import/export/assets/backups, client projection bridges, client snapshots, and
client generation/parsing/rendering.

Each candidate was independently re-verified against the cited code. The verifier
checked whether the path is live, whether existing work already mitigates it,
whether `leftover.md` already owns it, and how severe it is for a single-user
self-host install.
...
context-dependent findings are recorded at the end so they are not re-opened.

## Cross-Cutting Themes

Most findings are instances of five patterns. Fix the pattern when practical.

1. Server hot paths rebuild a broad in-memory `Database`. Lazy projection made
   wire payloads lean, but many server reads/writes still call
   `loadPersisted`/`loadPersistedWithMessages`. Cost scales with corpus size,
   not the row being touched. (H1, M1, M3, M4, M5, M10, L5, L6, L10, L18.)

2. Client hot paths still whole-corpus clone for narrow rollbacks. Callers use
   `currentChatStateSnapshot()` or `cloneJsonValue(DBState.db.characters)` when
   they need one scalar or row. Chat selection is the main high-severity case.
   (H2, M12, M13, M14, L31-L36, U4.)

3. Streaming re-does O(message) work per token. Each provider delta can trigger
   a full CBS+markdown+DOMPurify parse of the growing message. (H3.)

4. Outbound fetches lack preventive timeouts or abort propagation. Proxy,
   non-durable provider, and Lua fetch paths rely too much on defaults or client
   disconnect. (M6, M8, L20, L22-L25.)

5. Decompression and buffering lack preventive caps. Some paths check size after
   materialization, buffer until delimiters arrive, or leak resources on abort.
   (M9, M11, L11-L15, L27-L30.)
...
- The three landed workstreams remain valid. This audit found adjacent paths
  they did not cover: server reads still rebuild broad `Database` objects, and
  several client snapshot call sites still use broad clones.
- Findings tagged [known-leftover] are already tracked in
  `.archived-docs/deferred-work/leftover.md`. They remain listed for completeness; H1, H2, H3
  and most mediums are the priority.

## Suggested Remediation Order

1. H1 — guard on `message.length > 0`; removes a whole-corpus parse from
   chat-open and generation completion.
2. H3 — coalesce streaming renders.
3. H2 — add `ChatSelectionSnapshot`.
4. Server read narrowing — M1, M3, M4, M5, L5, L6, L10, L18.
5. Client clone narrowing — M12, M13, M14, L34, L35.
6. Stability foot-guns — M6, M8, M9, M11, L24, L27.
7. Known-leftover items stay gated unless real-corpus evidence changes priority.

## How To Reproduce / Verify

- Server stage timings: run with `RISU_PROTOCOL_METRICS=1`; watch
  `projection_response`, `databaseLoad*`, `asset_byte_read`, and
  `risusave_export`.
...
- Client per-token cost (H3): profile a long streamed response and watch
  `ParseMarkdown` / `risuChatParser` self-time.
