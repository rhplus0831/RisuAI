...
The v2 remediation workstream is closed and archived. There is no current
open `docs/plan/` workstream; this file remains as the historical closeout
summary and proof-command reference.
...
3. Closing proof: run the full closeout command set and record the final proof
   log.
4. Archive: moved the closed v2 plan to the archive and repointed the live
   gates.

## Closed-State Guardrails

- The plan is closed because both gates, `pnpm test`, `pnpm api:test`,
  `pnpm client-thinning:audit`, and both TypeScript checks are recorded in
  [`latest-verification.md`](latest-verification.md).
...
Phases 4-9 are green for focused suites, gates, final merged root/API proof,
