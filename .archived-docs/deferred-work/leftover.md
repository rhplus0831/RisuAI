...
Resolved history belongs in `git log` and `.archived-docs/`; permanent no-port
...
- Evidence: `server/fastify/src/prompt/luaRuntime.ts`;
  `.archived-docs/architecture-and-migration/client-thinning-closeout/phases/slices/slice-3b-lua/README.md`.
...
- Evidence: `server/fastify/src/routes/commands.ts` (the four routes);
  `.archived-docs/protocol-and-persistence/mutation-range-narrowing/phases/phase-6-message-free-ceiling.md`;
  `.archived-docs/protocol-and-persistence/mutation-range-narrowing/active-risk-analysis.md` (Tier 5).
