...
scope. The post-closeout audit slices and the alpha follow-up that
reopened additional projection-write blockers are archived; the
standing workstream continues at [`../client-thinning/`](../client-thinning/).
...
- **9-2 - Settings, presets, personas, loadouts.** Move lower-churn global
  ...
  - **9-2a - Scalar settings groups.** Split into secrets/providers,
    ...
    - **9-2a-ii - Manual scalar settings pages.** Complete. Added the shared
      ...
      the named manual settings surfaces. 
...
- **9-5 - Browser projection.** Turn the web client into a projection.
  ...
  - **9-5d - Residual command replacement sweep.** Replace remaining
    ...
    - **9-5d-i - Settings residual command sweep.** Complete. Routed
      ...
      fields on their dedicated command bridges. 
    - **9-5d-ii - 9-2 resource UI tails.** Complete. Audited prompt
      ...
      persona/translator delete selection payloads. 
    ...
    - **9-5d-iv - 9-4 extension UI/API tails.** Complete. Audited
      ...
      `moduleIntergration` with the settings bridge. 
...
These were satisfied for the original Fastify-served web closeout. The later alpha broad typecheck blocker is also closed (archived).
...
- Standing client-thinning workstream:
  [`../client-thinning/`](../client-thinning/)
