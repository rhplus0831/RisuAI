...
describe('server .risu fixture harness', () => {
  it('keeps the harness server-safe and detached from browser storage modules', () => {
    expect(harnessSource).not.toContain('localforage')
    expect(harnessSource).not.toContain('@tauri-apps')
    expect(harnessSource).not.toContain('database.svelte')
    expect(harnessSource).not.toContain('globalApi.svelte')
  })
  ...
  it('pins expected decoded shapes for legacy envelope fixtures', () => {
    ...
    for (const fixture of legacyFixtures) {
      expect(decodeLegacyFixtureEnvelope(fixture.bytes), fixture.name).toEqual(
