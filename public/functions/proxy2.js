...
async function fetchProxy(request) {
  ...
  const nheader = request.headers.get('risu-url')
    ? decodeURIComponent(request.headers.get('risu-url'))
    : null
  ...
  if (blocked_region.includes(region)) {
    ...
    const header =
      JSON.parse(decodeURIComponent(request.headers.get('risu-header') ?? 'null')) ??
      request.headers
